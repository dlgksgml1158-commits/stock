...
3. **숏스퀴즈 후보** — 공매도비중(Short Float) 20% 이상 **+ 오늘 상대거래량
   1.5배 이상**(구조적으로 스퀴즈 가능한데 실제로 오늘 거래가 몰리고 있는 것까지
   확인) 종목을, 공매도비중 x 커버일수(Short Ratio) x 오늘 상대거래량을 곱한
   "스퀴즈 점수" 내림차순으로. Short Float/Short Ratio는 거래소가 2주에 한 번
   집계하는 정적 지표라 이것만 보면 "오늘 아무 일도 없는" 종목이 계속 1위로
   나오는 문제가 있어서, 당일 거래량 조건을 필수로 걸었다. 레버리지·인버스 ETF는
   제외.
